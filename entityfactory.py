...
class EntityFactory:
    ...
    def get_entity(entity_name: str, position=(0, 0)):
        match entity_name:
            case 'lvl1':
                list_bg = []
                # Carrega as frames de animação do background, e posiciona elas lado a lado para criar um efeito de scrolling infinito
                for i in range(1, 8):
            ...
            case 'player1':
                ...
                return [Player('player1', position=(0, ALTURA_WIN // 2))]
            case 'player2':
                # Posiciona o player2 no canto esquerdo da tela, acima do player1
                return [Player('player2', position=(0, ALTURA_WIN // 3))]
            case 'inimigo1':
                # Posiciona o inimigo1 dentro da tela, perto da borda direita
                return [Enemy('inimigo1', position=(LARGURA_WIN+(LARGURA_WIN//4), random.randint(ALTURA_WIN // 3, ALTURA_WIN)))]
            case 'inimigo2':
                # Posiciona o inimigo2 dentro da tela, perto da borda direita
                return [Enemy('inimigo2', position=(LARGURA_WIN, random.randint(ALTURA_WIN//3, ALTURA_WIN - ALTURA_WIN//3)))]
            case _:
                return []